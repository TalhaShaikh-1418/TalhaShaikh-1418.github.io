/* ══════════════════════════════════════
   TALHAPORTFOLIO — script.js  (COMPLETE)
...
(function initCursor() {
  ...
  document.addEventListener("mouseover", (e) => {
    const t = e.target.closest(
      "a,button,.sn,.proj-shell,.glass,.stat-hex,.metric-card,.pp-card,.cta-primary,.cta-ghost,.ai-toggle-btn",
    );
...
(function initBoot() {
  ...
  (function bootDraw() {
    ...
    for (let x = 0; x < bcanv.width; x += gs) {
      bctx.beginPath();
      bctx.moveTo(x, 0);
      bctx.lineTo(x, bcanv.height);
      bctx.stroke();
    }
    for (let y = 0; y < bcanv.height; y += gs) {
      bctx.beginPath();
      bctx.moveTo(0, y);
      bctx.lineTo(bcanv.width, y);
      bctx.stroke();
    }
    for (let i = 0; i < 5; i++) {
...
const msgs = [
    ...
    ["init", "Loading project showcase..."],
    ["ok", "Web technologies initialized"],
    ["warn", "Establishing secure connection..."],
    ["ok", "Portfolio engine mounted"],
    ["init", "Rendering interactive interface..."],
    ["ok", "Animations and effects activated"],
...
function updateActiveNav() {
  ...
  let cur = "";
  secs.forEach((s) => {
    if (scrollY >= s.offsetTop - 230) cur = s.id;
  });
  links.forEach((a) =>
...
function initHeroCanvas() {
  ...
  class Streak {
    ...
    update() {
      this.y += this.spd;
      if (this.y - this.len > H) this.init();
    }
    draw() {
    ...
    (entries) => {
      entries.forEach((e) => {
        ...
        (function step(now) {
          ...
          valEl.textContent =
            Math.floor(ease * target) + (p >= 1 ? suffix : "");
          if (p < 1) raf(step);
        })(performance.now());
...
function initSkillGalaxy() {
  ...
  const SKILLS = [
    ...
    { name: "Java", color: "#f89820", r: 28 },
    { name: "Python", color: "#3572A5", r: 28 },
    { name: "PHP", color: "#777bb4", r: 24 },
    { name: "MySQL", color: "#00758f", r: 24 },
    { name: "Flask", color: "#aaaaaa", r: 22 },
    { name: "Git", color: "#f05032", r: 22 },
    { name: "AI/ML", color: "#00d4ff", r: 30 },
    { name: "GitHub", color: "#ffffff", r: 22 },
  ];
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    nodes.forEach((node) => {
      node.hov = Math.hypot(node.x - mx, node.y - my) < node.r + 12;

      if (node.hov) hovNode = node;
    });

    if (hovNode) {
      tooltip.style.left = `${hovNode.x + 18}px`;

      tooltip.style.top = `${hovNode.y - 12}px`;

      tooltip.textContent = hovNode.name;
  ...
  (function loop() {
    ...
    coreGlow.addColorStop(0, "rgba(0,212,255,0.35)");
    ...
    nodes.forEach((node) => {
      ...
      node.x = centerX + Math.cos(node.angle) * orbitX;

      node.y = centerY + Math.sin(node.angle) * orbitY;

      const radius = node.r + Math.sin(node.pulse) * 2 + (node.hov ? 6 : 0);
      ...
      ctx.strokeStyle = node.color + (node.hov ? "ff" : "88");

      ctx.lineWidth = node.hov ? 2 : 1;

      ctx.stroke();

      /* text */

      ctx.font = `${node.hov ? "700" : "500"} ${Math.round(
        radius * 0.52,
      )}px 'Share Tech Mono'`;

      ctx.fillStyle = node.hov ? "#ffffff" : node.color;
...
const SKILLS = [
  ...
  SKILLS.forEach((s) => {
    ...
    div.addEventListener("mousemove", (e) => {
      ...
      div.style.background = `radial-gradient(circle at ${e.clientX - r.left}px ${e.clientY - r.top}px,rgba(0,212,255,.1) 0%,rgba(0,212,255,.02) 60%)`;
    });
...
function initProjectTilt() {
  $$("[data-tilt]").forEach((card) => {
    card.addEventListener("mousemove", (e) => {
      ...
      card.style.transform = `perspective(1000px) rotateY(${x * 10}deg) rotateX(${-y * 8}deg) scale(1.015)`;
    });
...
function initAI() {
  ...
  bind(
    ...
    "keydown",
    (e) =>
      e.key === "Enter" && !e.shiftKey && (e.preventDefault(), handleQuery()),
  );

  // Consolidated Knowledge Base
  const responses = {
    about:
      "👨‍💻 TALHA(TALHA) is a Full-Stack Developer passionate about building scalable web applications, tackling complex engineering challenges, and creating interactive digital experiences.",
    skills:
      "🔥 TALHAis proficient in DSA , HTML5, CSS3, JavaScript, React.js, Python, Java, Spring Boot , MAVEN ,SQL, and the MERN Stack. He’s currently scaling into Next.js and AI dev.",
    projects:
      "🛠️ Talha's builds:\n🎬 *Movies Trending App*\n📄 *Smart Resume Builder* (AI-powered)\n📚 *Digital Library Manager*\n💻 *Personal Portfolio*",
    contact:
      "📬 Reach TALHAhere:\n🔗 LinkedIn: TALHA-847b61375\n💻 GitHub:0007RAM\n📧 Or use the contact form on this page!\n\n*(Note: Personal numbers, WhatsApp, and Discord IDs are private to prevent spam, but he is highly responsive via LinkedIn and Email!)*",
    leetcode:
      "📈 TALHAhas crushed 420+ LeetCode problems spanning Arrays, Graphs, and DP. Handle: 007RAM.",
    education:
      "🎓 TALHAis currently a 3rd-year B.Tech Computer Science student maintaining clean academic standing with zero active backlogs. He chose engineering out of a core passion for breaking down complex systems and building solutions from scratch.",
    hiring:
      "💼 Yes! TALHAis actively available for internships, full-time software engineering roles, and freelance collaborations. Drop a message via the contact form or LinkedIn to schedule an interview or introductory call!",
  };
  ...
  function getReply(q) {
    ...
    const personalRegex =
      /\b(wife|married|marriage|girlfriend|boyfriend|relationship|age|old|salary|money|income|pay|cash|religion|caste|political|politics|party|live|location|address|home)\b/i;
    if (personalRegex.test(q)) {
      return `FAAAAAHHHH! 💀💀💀💀\n\nNice try, but Talha's personal life/demographics are encrypted. Let's stick to his code, projects, and career skills. Stay focused!`;
    }

    // 2. TARGETED GUARDRAIL B: Academic Dishonesty / Malicious Requests
    const exploitRegex =
      /\b(hack|hacking|crack|cracking|cheat|cheating|exam|assignment|homework|do my project|write my project)\b/i;
    if (exploitRegex.test(q)) {
      return `FAAAAAHHHH! 💀💀💀💀\n\nI am an engineering portfolio assistant, not a dark-web client or a homework bot. TALHAbuilds robust, ethical software. Check out his real GitHub projects instead!`;
    }

    // 3. High-Priority Intent Routing (Greetings)
    if (/^(hi|hello|hey|hii|hola|who are you|yo|greetings)/i.test(q)) {
      return `👋 Yo! I'm RAM.AI. Ask me about Talha's professional projects, engineering skills, or LeetCode stats!`;
    }
    ...
    // 5. Absolute Fallback
    return `FAAAAAHHHH! 💀💀💀💀\n\nI have no idea what you just said. My knowledge base only covers Talha's professional portfolio. Ask about his projects or skills!`;
  }
